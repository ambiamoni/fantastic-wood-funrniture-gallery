import React from "react";

const CardHome = () => {
  return (
    <div className="my-20 grid  grid-cols-2">
      <div className="hero text-left pl-20 ">
        <div className="hero-contentflex grid grid-cols-1 ">
          <div>
            <img src="https://media.istockphoto.com/id/1352177016/photo/modern-living-room-interior-with-air-conditioner-orange-sofa-and-green-armchair.jpg?b=1&s=170667a&w=0&k=20&c=xK3Po9EHKfU5zN3tPWr6kbo_5jelzw97uo3dhCMkspY=" />
          </div>

          <div className="mt-10">
            <h1 className="text-3xl font-semibold text-[#547e7e]">
              HOW TO CHOOSE WOOD FOR <br></br>FURNITURE MAKING
            </h1>
            <p className="py-6">
              Are you interested in building your own furniture but do not know
              where to start? At Birchwood Furniture, we know everything when it
              comes to furniture–including which building materials are best.
              While almost any type of wood can be used to build furniture, some
              woods are a better choice depending on your goals, your budget
            </p>
            <button className="pt-3 pb-3 pr-10 pl-10 bg-[#547e7e] border-0 text-white hover:bg-[#7d9c9c]">
              LEARN MORE
            </button>
          </div>
        </div>
      </div>
      <div className="hero text-left pl-20 ">
        <div className="hero-contentflex grid grid-cols-1 ">
          <div>
            <img src="https://media.istockphoto.com/id/1007240024/photo/book-on-a-black-and-white-pouf-in-the-middle-of-a-bright-terrace-with-a-rattan-corner-sofa.jpg?s=612x612&w=is&k=20&c=11SefI6XMfHL3aLCyxitwa_9HuEMuAQkkPle5N0oh_c=" />
          </div>

          <div className="mt-10">
            <h1 className="text-3xl font-semibold text-[#547e7e]">
              HOW TO CHOOSE WOOD FOR <br></br>FURNITURE MAKING
            </h1>
            <p className="py-6">
              Are you interested in building your own furniture but do not know
              where to start? At Birchwood Furniture, we know everything when it
              comes to furniture–including which building materials are best.
              While almost any type of wood can be used to build furniture, some
              woods are a better choice depending on your goals, your budget
            </p>
            <button className="pt-3 pb-3 pr-10 pl-10 bg-[#547e7e] border-0 text-white hover:bg-[#7d9c9c]">
              LEARN MORE
            </button>
          </div>
        </div>
      </div>
      
      <div className="hero text-left pl-20 ">
        <div className="hero-contentflex grid grid-cols-1 ">
          <div>
            <img src="https://birchwoodfurniture.ca/wp-content/uploads/2022/08/couple-arranging-furniture-scaled.jpg" />
          </div>

          <div className="mt-10">
            <h1 className="text-3xl font-semibold text-[#547e7e]">
              HOW TO CHOOSE WOOD FOR <br></br>FURNITURE MAKING
            </h1>
            <p className="py-6">
              Are you interested in building your own furniture but do not know
              where to start? At Birchwood Furniture, we know everything when it
              comes to furniture–including which building materials are best.
              While almost any type of wood can be used to build furniture, some
              woods are a better choice depending on your goals, your budget
            </p>
            <button className="pt-3 pb-3 pr-10 pl-10 bg-[#547e7e] border-0 text-white hover:bg-[#7d9c9c]">
              LEARN MORE
            </button>
          </div>
        </div>
      </div>
      <div className="hero text-left pl-20 ">
        <div className="hero-contentflex grid grid-cols-1 ">
          <div>
            <img src="https://media.istockphoto.com/id/465559373/photo/old-wood-background.jpg?b=1&s=170667a&w=0&k=20&c=x4oxoo5W_QY51yFdCEg_rM6YgmvhhybsYTrGpEDF2QQ=" />
          </div>

          <div className="mt-10">
            <h1 className="text-3xl font-semibold text-[#547e7e]">
              HOW TO CHOOSE WOOD FOR <br></br>FURNITURE MAKING
            </h1>
            <p className="py-6">
              Are you interested in building your own furniture but do not know
              where to start? At Birchwood Furniture, we know everything when it
              comes to furniture–including which building materials are best.
              While almost any type of wood can be used to build furniture, some
              woods are a better choice depending on your goals, your budget
            </p>
            <button className="pt-3 pb-3 pr-10 pl-10 bg-[#547e7e] border-0 text-white hover:bg-[#7d9c9c]">
              LEARN MORE
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default CardHome;
